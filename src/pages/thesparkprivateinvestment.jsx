...
export default function Investor() {
    ...
    // ============ PRIVATE ACCESS CODE STATE ============
    const [privateCode, setPrivateCode] = useState('');
    const [privateAuthenticated, setPrivateAuthenticated] = useState(false);
    const [checkingPrivateCode, setCheckingPrivateCode] = useState(true);
...
// ✅ 1. Check private access code from URL ONLY
        useEffect(() => {
            const urlCode = searchParams.get('code');
            
            // ✅ If code is in URL, grant access
            if (urlCode === 'PRIVATE2026') {
                setPrivateAuthenticated(true);
                toast.success('✅ Private access granted!');
            } else {
                // ✅ Always show code entry screen
                setPrivateAuthenticated(false);
            }
            
            setCheckingPrivateCode(false);
        }, []);
    ...
    // ============================================================
    // ✅ AFTER ALL HOOKS ARE CALLED, NOW YOU CAN DO CONDITIONAL RETURNS
    // ============================================================

    // ✅ Show loading while checking private code
    if (checkingPrivateCode) {
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="animate-spin rounded-full h-12 w-12 border-4 border-gray-300 border-t-spark-500"></div>
            </div>
        );
    }

    // ✅ If not authenticated with private code, show code entry
   if (!privateAuthenticated) {
    ...
    const handleAccess = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await api.post('/private/verify-access', {
                code: privateCode
            });
            ...
            if (response.data.success) {
                ...
                toast.error('❌ Invalid access key. Try again.');
            }
        } catch (error) {
            console.error('Access error:', error);
            toast.error(error.response?.data?.error || '❌ Invalid access key');
        } finally {
            setIsLoading(false);
        }
    };
   ...
   return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-amber-50 via-orange-50 to-white relative overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden">
            ...
            {[...Array(20)].map((_, i) => (
                <div
                    key={i}
                    className="absolute w-1 h-1 bg-spark-400/30 rounded-full animate-float"
                    style={{
                        top: `${Math.random() * 100}%`,
        ...
        <div className="relative z-10 w-full max-w-md">
            <div className="relative group">
                ...
                <div className="absolute -inset-1 bg-gradient-to-r from-spark-400 via-spark-500 to-amber-400 rounded-2xl blur-xl opacity-75 group-hover:opacity-100 transition duration-1000 group-hover:duration-200 animate-gradient"></div>
                
                <div className="relative bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-white/50">
                    {/* Header */}
                    <div className="text-center mb-8">
                        <div className="relative inline-block">
                            <div className="relative w-24 h-24 rounded-full flex items-center justify-center mx-auto shadow-2xl animate-bounce-slow">
                                <span className="text-4xl">🔥</span>
                            </div>
                        </div>
                        
                        <h2 className="text-3xl font-bold text-gray-900 mt-6 mb-2 tracking-tight">
                            Welcome to
                            <span className="block text-transparent bg-clip-text bg-gradient-to-r from-spark-500 to-amber-500">
                                TheSpark Inner Circle
                            </span>
                        </h2>
                        <p className="text-gray-500 text-sm font-light">
                            You've been invited to something exclusive
                        </p>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-3.5 bg-white/50 border border-gray-200 rounded-xl text-gray-800 placeholder-gray-400 focus:ring-2 focus:ring-spark-500 focus:border-transparent transition-all duration-300 outline-none"
                                onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                        handleAccess();
                                    }
                        ...
                        <button
                            ...
                            disabled={isLoading}
                            className="relative w-full py-3.5 rounded-xl font-semibold text-white overflow-hidden group/btn transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            <span className="absolute inset-0 bg-gradient-to-r from-spark-500 via-spark-600 to-amber-500 animate-gradient"></span>
                            <span className="absolute inset-0 bg-gradient-to-r from-spark-500 via-spark-600 to-amber-500 opacity-0 group-hover/btn:opacity-100 transition-opacity duration-500 blur-xl"></span>
                        ...
                        <div className="flex items-center justify-center gap-2 text-xs text-gray-400">
                            ...
                            <span>Secure connection • 256-bit encrypted</span>
                        </div>

                        <p className="text-xs text-gray-400 text-center mt-2">
                            Contact your sponsor for the access key
                        </p>
                    </div>
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-200">
                        <div className="flex justify-center gap-6">
                            <Link 
                                to="/dashboard" 
                                className="text-sm text-gray-500 hover:text-spark-500 transition-colors duration-300 flex items-center gap-1.5"
                            >
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                        <div className="flex justify-center gap-2">
                            {[...Array(5)].map((_, i) => (
                                <div
                                    key={i}
                                    className="w-1.5 h-1.5 rounded-full bg-gradient-to-r from-spark-400 to-amber-400"
                                    style={{
    ...
    // ============================================================
    // ✅ REST OF YOUR COMPONENT (STEPS, HANDLERS, RETURNS)
    // ============================================================
            ...
            <div className="min-h-screen bg-gray-50 py-12">
                <div className="max-w-3xl mx-auto px-4">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <p className="text-xs text-blue-600 mt-3">🔓 Admin Mode - Security bypassed</p>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8">
                        <h3 className="font-semibold text-gray-900 text-lg mb-4">Express Interest</h3>
                        <p className="text-sm text-gray-500 mb-6">This is a private invitation. All information will be kept strictly confidential.</p>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <label className="text-sm text-gray-600">
                                        I confirm that this is a private investment invitation and I understand the profit-sharing terms.
                                        <br />
                                        <span className="text-xs text-gray-400">Investment is at risk. No guaranteed returns.</span>
                                        <br />
                                        <button
                                            type="button"
                                            onClick={() => setShowTermsModal(true)}
                            ...
                            <p className="text-xs text-gray-400 text-center mt-2">👑 Admin Mode - Security bypassed</p>
                        </div>
                    </div>
                </div>
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="space-y-4 text-gray-700 text-sm leading-relaxed">
                                    ...
                                    <div className="bg-gray-50 rounded-xl p-4 mt-4">
                                        <p className="text-xs text-gray-500">Last updated: June 24, 2026</p>
                                    </div>
                                </div>
                            </div>
            ...
            <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-xs text-gray-400 mt-1">Code expires in 24 hours • One-time use</p>
                        {user && (
                            <p className="text-xs text-green-600 mt-2">✅ Logged in as: <span className="font-medium">{user.email}</span></p>
                        )}
                        {!user && (
                            <p className="text-xs text-blue-600 mt-2">💡 No account needed! Just enter your code below.</p>
                        )}
                    </div>
                    ...
                    <form onSubmit={handleVerifyCode} className="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                autoFocus
                            />
                            <p className="text-xs text-amber-600 mt-2">⚠️ This code can only be used once and expires in 24 hours</p>
                        </div>
                    ...
                    <div className="mt-6 text-center">
                        <p className="text-xs text-gray-400">This is a private investment opportunity. <br />Contact Gideon for your access code.</p>
                    </div>
                </div>
            </div>
        );
                ...
                <div className="max-w-2xl mx-auto px-4">
                    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
                        <div className="text-center mb-8">
                            <div className="text-4xl mb-3">📝</div>
                            <h2 className="text-2xl font-bold text-gray-900">Your Details</h2>
                            <p className="text-gray-500 mt-2">Please provide your information to verify your identity</p>
                            {investorName && <p className="text-sm text-spark-600 mt-1">Welcome, {investorName}!</p>}
                            <p className="text-xs text-gray-400 mt-3">🔒 Your information is encrypted and secure</p>
                        </div>
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="space-y-4 text-gray-700 text-sm leading-relaxed">
                                    <h4 className="text-lg font-semibold text-gray-900">1. Investment Structure</h4>
                                    <p>This is a <strong>private profit-sharing investment</strong> opportunity...</p>
                                    <ul className="list-disc pl-6 space-y-2">
                                        <li><strong>Phase 1 (Years 1-2):</strong> 100% of profits go to investors until they receive their full initial investment back.</li>
                                        <li><strong>Phase 2 (Years 3-5):</strong> After capital is returned, 30% of profits are distributed to investors.</li>
                                    </ul>
                                    ...
                                    <h4 className="text-lg font-semibold text-gray-900 pt-4">3. Confidentiality</h4>
                                    <p>All information provided about this investment opportunity is <strong>strictly confidential</strong>.</p>
                                    ...
                                    <h4 className="text-lg font-semibold text-gray-900 pt-4">6. Privacy</h4>
                                    <p>We collect and process your personal information solely for the purpose of managing your investment interest.</p>
            ...
            <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8">
                    <div className="text-center mb-8">
                        <div className="text-5xl mb-4">📧</div>
                        <h2 className="text-2xl font-bold text-gray-900">Verify Your Email</h2>
                        <p className="text-gray-500 mt-2">We sent a 6-digit code to</p>
                        <p className="font-medium text-gray-900">{formData.email}</p>
                        <p className="text-xs text-gray-400 mt-3">Code expires in {otpTimer} minute{otpTimer !== 1 ? 's' : ''}</p>
                    </div>
                    ...
                    <div className="mt-4 text-center">
                        {canResend ? (
                            ...
                            <p className="text-sm text-gray-400">Resend available in {otpTimer} minute{otpTimer !== 1 ? 's' : ''}</p>
                        )}
                    </div>
            ...
            <div className="min-h-screen bg-gray-50 py-12">
                <div className="max-w-3xl mx-auto px-4">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-xs text-gray-400 mt-3">🔒 Session expires in {sessionExpiry} minutes</p>
                    </div>
